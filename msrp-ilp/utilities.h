...
void checkILPsolution()
{
    ...
    for(int i=0;i<tasks.size();i++)
    {
        if(tasks[i].ilpR > tasks[i].b.totRespTime && tasks[i].ilpR-tasks[i].b.totRespTime>0.001)
            debug10<<"response time error"<<endl;
        if(tasks[i].ilpB > tasks[i].b.lmsrp && tasks[i].ilpB-tasks[i].b.lmsrp>0.001)
            debug10<<"response time error"<<endl;
    }
